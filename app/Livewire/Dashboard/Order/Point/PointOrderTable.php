...
class PointOrderTable extends BaseTable
{
    ...
    {
        return $this->model::query()->with(['branch', 'customer'])->where('type', 'point')
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('Manager'), 'manager_id')
                ->format(function ($value, $column, $row) {

                    return ($column->manager) ? $column->manager->name : __('No Manager');
                })
                ->searchable()
                ->sortable(),

            Column::make(__('Product'), 'product_id')
                ->format(function ($value, $column, $row) {
                    return $column->product?->title;
                })
                ->searchable()
                ->sortable(),
